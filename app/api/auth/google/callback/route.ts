...
export async function GET(request: Request) {
  ...
  try {
    ...
    // The query flag tells the service worker to bypass its cached app shell
    // once, so the newly authenticated page is rendered immediately.
    const response = new Response(null, { status: 302, headers: { Location: new URL("/?signed_in=1", request.url).toString() } });
